...
|-------|-----------------------------|---------------------|
| **Nomad** | No position, quiet unofficial use, or a defensive ban. | Set a direction and a reason — one sentence on *why*, tied to a real outcome. |
| **Observer** | A working group, a policy in draft, lots of watching. | Authorise a low-stakes pilot with a named owner. |
| **Dabbler** | Scattered, individual use. Wins don't spread; risk isn't governed. | A light policy + a shared win, so good practice becomes standard. |
| **Integrator** | Several functions save real time. Uneven, undocumented, person-dependent. | Make it institutional and equitable — not reliant on the enthusiasts. |
| **Architect** | You can see a redesigned service or model but haven't committed to building it. | Decide the organisation is one that builds, and resource it. |
| **Innovator** | AI is built into what you offer and how you operate — a visible advantage. | Govern it as you scale; choose which bets are worth it. |

**The gap that stalls organisations** is usually between the leader's stage and the workforce's. An Integrator leader with a Nomad workforce mandates a tool nobody adopts. A Nomad leader with an Integrator workforce has ungoverned use and no audit trail. Diagnose both before you act — the fix is different in each direction.
...
The one metric leaders skip: **where does the saved time go?** If AI buys back hours and they vanish into more busywork, you've spent money to run faster in place. The [board briefing and metrics](../templates/board-briefing-and-metrics.md) makes that question — and a few leading indicators — visible to the people who fund the programme.
